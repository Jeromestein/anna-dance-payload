# Historical billing checkpoints — superseded

This is an archived implementation record, not the current checklist. See [Account Billing](account-billing.md) for the current design and [live acceptance](../operations/stripe-payment-refund-testing.md) for remaining work.

# Account Billing

Decision date: September 9, 2026
Status: Stripe checkout, payment/refund synchronization, and Admin refund code implemented locally on September 10; database migration applied. Provider-backed testing, configuration, historical import, and deployment remain unfinished.

The current implementation and live verification steps are in [Stripe live payment and full-refund verification](../operations/stripe-payment-refund-testing.md). The September 9 checklist below records the earlier frontend-only checkpoint; the September 10 acceptance checklist in that guide supersedes its implementation statuses.
...
The Cal.com webhook persists signed schedule identity; booking events alone never generate Paid bills. Stripe payment/refund notifications retrieve provider evidence, match a reserved bill or signed Cal booking ID plus attendee email, and atomically update billing. Unassociated events require replay or verified Staff import. Checkout uses server-stored bill amounts; do not reuse an unrelated Payment Link. Configuration and provider-backed acceptance remain required before live initiation.

## Required payment synchronization checklist

September 9 checkpoint (see the September 10 implementation and acceptance guide above for current progress). The user explicitly deferred implementation for later; these items are required, not optional enhancements. Database availability and manual billing controls do not mean payment integration is complete.
...
- [ ] TODO: Verify authenticated Admin against existing genuine live payments and intended actual refunds. Sandbox acceptance is not required. Cover pending/failure, duplicate clicks, stale data, and permissions locally; never manufacture live transactions just for testing.

#### How to test the frontend

1. Use the site version with the refund interface, sign in as staff at `/admin`, then open **Student → Jason → Billing**. The demo works without Stripe configuration; provider-backed tests require the September 10 guide.
...
5. Use **Restart demo** to repeat. A demo does not verify the provider integration. Follow the current Stripe testing guide for a provider-backed payment and refund. **Record completed refund** only records money already returned externally and is unavailable for provider-synchronized bills.
